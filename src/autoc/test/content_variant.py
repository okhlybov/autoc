...
x = Type(type := Variant("content_variant", {"amount": "int", "object": arc}))
...
x.unit(f"{type.set_object}(): set the composite alternative", f"""
  {p.definition};
  {arc.create(p, 7)};
  {type.set_object(t, p)};
  TEST_FALSE( {type.is_empty(t)} );
  TEST_FALSE( {type.is_amount(t)} );
  TEST_TRUE( {type.is_object(t)} );
  TEST_EQUAL( *{type.get_object_view(t)}->value, 7 );
  {arc.destroy(p)};
""")
...
x.unit(f"{type.set_object}(): switch between the same composite alternative", f"""
  {p.definition};
  {arc.create(p, 7)};
  {type.set_object(t, p)};
  {type.set_object(t, p)};
...
x.unit(f"{type.equal}(): compare variants sharing the same composite value", f"""
  {p.definition};
  {arc.create(p, 9)};
  {type.set_object(t1, p)};
  {type.set_object(t2, p)};
  {arc.destroy(p)};
  TEST_TRUE( {type.equal(t1, t2)} );
  TEST_TRUE( {type.is_object(t1)} );
  TEST_TRUE( {type.is_object(t2)} );
""")
...
x.unit(f"{type.copy}(): copy variant holding the composite alternative", f"""
  ...
  TEST_TRUE( {type.equal(t1, t2)} );
  TEST_TRUE( {type.is_object(t2)} );
  TEST_EQUAL( *{type.get_object_view(t2)}->value, 13 );
""")
...
x.unit(f"{type.move}(): move variant holding the composite alternative", f"""
  ...
  {type.move(t2, t1)};
  TEST_TRUE( {type.is_object(t2)} );
  TEST_EQUAL( *{type.get_object_view(t2)}->value, 17 );
  TEST_NULL( {type.get_object_view(t1)} );
""")