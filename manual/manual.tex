...
Returns a new heap-allocated container constructed with \meth{void \st Ctor()}.
...
Returns non-zero value when \emph{index} is a valid index, that is \emph{index} $\in$ [0, \meth{\st Size(\sv)}), and zero otherwise.
...
The container takes ownership over the new element \ev.
