...
module Decorator
  ...
  # Prefix for the generated identifier; can be aliased to name_c, for example
  # attr_reader :prefix_c

  # Per type decorator instance; if nil, a globally specified decorator is used
  # @decorator
  ...
  CAMEL_CASE = -> (type, symbol, abbreviate: false, **kws) {
    ...
    id = type.prefix_c + id.split('_').collect{ |s| s[0].upcase << s[1..-1] }.join
    # Carry over the method name's leading underscore only if the prefix is not in turn underscored
    (_ && !type.prefix_c.start_with?('_') ? Regexp.last_match(1) + id : id) + trail
  }
  ...
  SNAKE_CASE = -> (type, symbol, abbreviate: false, **kws) {
    ...
    id = abbreviate ? "#{type.prefix_c}#{id[0]}" : "#{type.prefix_c}_#{id}"
    # Carry over the method name's leading underscore only if the prefix is not in turn underscored
    (_ && !type.prefix_c.start_with?('_') ? Regexp.last_match(1) + id : id) + trail
  }
